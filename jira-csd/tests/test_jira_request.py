...
class JiraRequestTests(unittest.TestCase):
    ...
    def test_request_jira_multipart_posts_files_with_exact_headers(self):
        env_path = self.write_env(
            "\n".join(
                [
                    "jira_user=user@example.com",
        ...
        tmpdir = tempfile.TemporaryDirectory()
        self.addCleanup(tmpdir.cleanup)
        image_path = Path(tmpdir.name) / "screen.png"
        log_path = Path(tmpdir.name) / "agora.log"
        image_path.write_bytes(b"\x89PNG\r\n\x1a\n")
        log_path.write_text("example log", encoding="utf-8")
        response = FakeResponse(
            [
                {"filename": "screen.png"},
                {"filename": "agora.log"},
        ...
        payload = self.module.request_jira_multipart(
            "/rest/api/2/issue/CSD-10001/attachments",
            file_paths=[image_path, log_path],
            env_path=env_path,
            token_fetcher=lambda _: "token-123",
            connection_factory=fake_connection_factory,
        ...
        connection = captured["connection"]
        headers = dict(connection.headers)
        body = connection.sent_body.decode("utf-8", errors="replace")

        self.assertEqual(payload[0]["filename"], "screen.png")
        self.assertEqual(connection.request_method, "POST")
        self.assertEqual(
            connection.request_target,
            "/rest/api/2/issue/CSD-10001/attachments",
        )
        self.assertEqual(headers["Accept"], "application/json")
        self.assertEqual(
        ...
        self.assertIn('name="file"; filename="screen.png"', body)
        self.assertIn('name="file"; filename="agora.log"', body)
